...
function isVulnerableAfterMove(boardState, piece, toCell) {
    var potentialJumps = get_available_piece_moves(boardState, piece, -piece.state);
    for (var i = 0; i < potentialJumps.length; i++) {
        if (potentialJumps[i].move_type === "jump") {
            return true;
        }
    }
    return false;
...
function movePiece(boardState, piece, fromCell, toCell, moveNum) {
    ...
    if (jumpedPiece != null) {
        ...
        for (var i=0; i<more_moves.length; i++) {
            ...
            if (more_move.move_type === "jump" && !isVulnerableAfterMove(boardState, piece, more_move.to)) {
...
function drawBoard(origin, cellWidth, boardCanvas) {
	...
	boardCanvas.append("g")
				...
				.attr("class", function(d) { return "cell " + ((d.row % 2 === d.col % 2) ? "white" : "black"); });
	...
	var pieceGroup = boardCanvas.append("g")
				.selectAll("g")
				.data(pieces)
				.enter().append("g")
				.attr("class", function(d) { return "piece " + (d.state == red ? "red" : "black"); })
				.call(drag);

	pieceGroup.append("circle")
				.attr("class", "outer")
				.attr("r", cellWidth / 2)
...
function moveCircle(cell, moveNum) {
	var cellCoordinates = mapCellToCoordinates(board_origin, cell_width, cell);
	currentBoard.delay = (moveNum * 500) + 500;
	d3.selectAll("g.piece").each(function(d,i) {
		if (d.col === cell.col && d.row === cell.row){
			d3.select(this).selectAll("circle")
			.transition()
			.delay(500 * moveNum)
...
class MCTSNode {
    ...
    isFullyExpanded() {
        return this.children.length === get_available_moves(this.board.turn, this.board).length;
    }
    ...
    expand() {
        const moves = get_available_moves(this.board.turn, this.board);
        const untriedMoves = moves.filter(move => !this.children.some(child => child.move === move));
        const move = untriedMoves[Math.floor(Math.random() * untriedMoves.length)];
        const newBoard = copy_board(this.board);
        const pieceIndex = getPieceIndex(newBoard.pieces, move.from.row, move.from.col);
        const piece = newBoard.pieces[pieceIndex];
        movePiece(newBoard, piece, move.from, move.to, 1);
        const childNode = new MCTSNode(newBoard, this, move);
...
function mcts(board, iterations) {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        // Simulation
        let simulationBoard = copy_board(node.board);
        let result = simulateRandomPlayout(simulationBoard);
...
function simulateRandomPlayout(board) {
    while (true) {
        const moves = get_available_moves(board.turn, board);
        if (moves.length === 0) {
            return board.turn === player ? 0 : 1; // 1 if computer wins, 0 if player wins
        }
        const move = moves[Math.floor(Math.random() * moves.length)];
        const pieceIndex = getPieceIndex(board.pieces, move.from.row, move.from.col);
        const piece = board.pieces[pieceIndex];
        movePiece(board, piece, move.from, move.to, 1);
        board.turn = -board.turn;
    }
}

function computerMove() {
    const iterations = 1000; // Adjust the number of iterations for performance
    const selected_move = mcts(currentBoard, iterations);
