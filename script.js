	...
	(0, _jquery2.default)(document).ready(function () {
		...
		function systemResponse() {
			...
			if (!bestMove) {
				...
				var fromSquare = bestMove[0];
				var toSquare = bestMove[bestMove.length - 3];
				
				// Animate the move directly to the final position
				animateMove(fromSquare, toSquare, function() {
		...
		// Function to animate a single move
		// fix problem w/ computer not animating multiple jumps.
		function animateMove(fromSquare, toSquare, callback) {
			var fromCell = (0, _jquery2.default)('#tblBoard td').eq(fromSquare);
			var toCell = (0, _jquery2.default)('#tblBoard td').eq(toSquare);
			var piece = fromCell.children().detach();
			piece.appendTo('body').css({
				position: 'absolute',
				top: fromCell.offset().top,
				left: fromCell.offset().left
			}).animate({
				top: toCell.offset().top,
				left: toCell.offset().left
			}, 1000, function() {
		...
		function squareClick(sq) {
			...
			if (selectedSquare == null) {
				...
				var fromRow = Math.floor(selectedSquare / 10);
				var toRow = Math.floor(sq / 10);
				var man = game.Position.Nodes[selectedSquare];
		
				// Check if the move is backwards
				if ((man === 1 && toRow <= fromRow) || (man === -1 && toRow >= fromRow)) {
					alert("Invalid Move: You can't move backwards");
				} else {
					// Check if the move is a capture move
					var isCaptureMove = Math.abs(fromRow - toRow) > 1;
		
					// Check if the capture move is backwards
					if (isCaptureMove && ((man === 1 && toRow <= fromRow) || (man === -1 && toRow >= fromRow))) {
						alert("Invalid Move: You can't capture backwards");
					} else {
		...
		// Helper function to format the move history
		function formatMove(move) {
			const startSquare = move[0];
			const endSquare = move[move.length - 3];
			const isCapture = move.length > 6; // Check if the move is a capture move
		
			const startRow = Math.floor(startSquare / 10);
			const startCol = startSquare % 10;
			const endRow = Math.floor(endSquare / 10);
			const endCol = endSquare % 10;
		
			const start = `E${startRow * 10 + startCol}`;
			const end = `E${endRow * 10 + endCol}`;
		
			return isCapture ? `${start} X ${end}` : `${start} - ${end}`;
		}
		...
		function createView() {
			try {
				...
				for (var m = 0; m < game.MoveHistory.length; m++) {
					try {
						var theRow = document.createElement("tr");
						var theCell1 = document.createElement("td");
						theCell1.innerHTML = formatMove(game.MoveHistory[m]);
				...
				for (var row = 0; row < 10; row++) {
					...
					for (var col = 0; col < 10; col++) {
						...
						td.setAttribute("bgColor", validSq ? "whitesmoke" : "#4959b5");
						td.style.height = "50px"; // Adjusted cell height
						td.style.width = "50px"; // Adjusted cell width
						...
						if (validSq) {
							...
							if (man !== 0) {
								...
								img.style.height = "40px"; // Adjusted image height
								img.style.width = "40px"; // Adjusted image width
								var imgSrc = "";
								switch (man.toString()) {
									case "1":
